...
namespace STBEverywhere_back_APICompte.Services
{
    public class AgiosService
    {
        ...
        {
            const decimal TAUX_AGIOS_ANNUEL = 0.18m; // 12% fixe pour tous les comptes
           ...
           /* var dateCalcul = DateTime.Now;
            ...
            var dateCalcul = DateTime.Now;
            var dateDebutMois = new DateTime(dateCalcul.Year, dateCalcul.Month, 1).AddMonths(-1); // début du mois précédent
            var dateFinMois = dateDebutMois.AddMonths(1).AddDays(-1); // fin du mois précédent
            ...
            try
            {
                ...
                {
                    var agiosDejaAppliques = await _context.FraisComptes
        .AnyAsync(f => f.RIB == compte.RIB
                       && f.type == "PRLV AGIOS Découvert MENSUELS"
                       && f.Date.Year == dateDebutMois.Year
                       && f.Date.Month == dateDebutMois.Month);

                    if (agiosDejaAppliques)
                    {
                        _logger.LogInformation($"Agios déjà appliqués pour le compte {compte.RIB} pour le mois {dateDebutMois.Month}/{dateDebutMois.Year}.");
                        continue;
                    }
        ...
        //pour le test 
        public async Task CalculerEtAppliquerAgiosPourMois(DateTime dateCible)
        {
            ...
            _logger.LogInformation($"Début du calcul des agios pour le mois de {dateCible:MMMM yyyy}");

            // Cible : le mois de la date donnée
           /* var dateDebutMois = new DateTime(dateCible.Year, dateCible.Month, 1);
            var dateFinMois = dateDebutMois.AddMonths(1).AddDays(-1);
            var dateCalcul = DateTime.Now; // date réelle d’application des frais*/
            ...
            try
            {
                ...
                foreach (var compte in comptes)
                {
                    ...
                    if (totalAgios > 0)
                    {
                        ...
                        _context.FraisComptes.Add(new FraisCompte
                        {
                            type = $"PRLV AGIOS Découvert MENSUELS ({dateDebutMois:MM/yyyy})",
                            Date = dateCalcul,
                            Montant = montantFinal,
                            RIB = compte.RIB
                        });

                        _logger.LogInformation($"Agios de {totalAgios} appliqués au compte {compte.RIB} pour la période {dateDebutMois:dd/MM/yyyy} - {dateFinMois:dd/MM/yyyy}");

                        // Clôturer la période de découvert active
                        var periodeActive = compte.PeriodesDecouvert.FirstOrDefault(p => p.DateFin == null);
                        if (periodeActive != null)
                        {
                            periodeActive.DateFin = dateCalcul;
                ...
                await _context.SaveChangesAsync();
                _logger.LogInformation("Calcul des agios (manuel) terminé avec succès");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Erreur lors du calcul manuel des agios");
                throw;
            }
        }
